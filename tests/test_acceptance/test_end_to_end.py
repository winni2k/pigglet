...
def test_single_mutation_one_sample_creates_trivial_graph(tmpdir):
    ...
    # when
    result = runner.invoke(cli.cli, ['infer', str(vcf_file), str(prefix)])
...
def test_converts_mutation_tree_to_phylogenetic_tree(tmpdir, invoke):
    ...
    in_h5 = tmpdir / 'in.h5'
    g = nx.relabel_nodes(nx.balanced_tree(r=2, h=2, create_using=nx.DiGraph), lambda n: n - 1)
    ...
    assert sorted(set(new_g.edges())) == sorted({('-1', '0'),
                                                 ('-1', '3'), ('3', '1'),
                                                 ('-1', '4'), ('4', '2')})
    with h5py.File(out_prefix + '.h5', 'r') as fh:
        assert list(fh['map_phylogenetic_tree/mutation_attachments/mutation_ids']) == [0, 1, 5]
        assert list(fh['map_phylogenetic_tree/mutation_attachments/attachments']) == [3, 4, 4]
