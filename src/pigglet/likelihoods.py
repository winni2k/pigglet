...
class TreeLikelihoodCalculator:
    ...
    def attachment_log_like(self):
        ...
        self._update_attachment_and_summed_likes()
    ...
    def refresh_attachment_marginalized_sample_log_likelihoods(self):
        """The partial update of marginalized sums suffers from moderate numerical errors.
        This means that the sum will drift from it's true value over subsequent updates.
        This method causes the summed attachment log likelihoods to be recalculated from
        all attachment likelihoods on the next call of
        attachment_marginalized_sample_log_likelihoods().
        """
        self._summed_attachment_log_like = None
    ...
    def mutation_probabilities(self, attach_prob):
    ...
    def _update_attachment_log_like_from(self, start):
        if start == self.root:
            ...
            yield start + 1, current_log_like
            self._attachment_log_like[start + 1] = current_log_like
        else:
            parent = list(self.g.pred[start])[0]
            current_log_like = (self._attachment_log_like[parent + 1]
        ...
        for u, v, label in nx.dfs_labeled_edges(self.g, start):
            ...
            if label == 'forward':
                ...
                yield v + 1, current_log_like
                self._attachment_log_like[v + 1] = current_log_like
            elif label == 'reverse':
                current_log_like -= diffs.pop()
            else:
                raise ValueError(f'Unexpected label: {label}')

    def _update_attachment_and_summed_likes(self):
        if self.has_changed_nodes():
            if self._summed_attachment_log_like is None:
                for node in self._changed_nodes:
                    for _ in self._update_attachment_log_like_from(node):
                        pass
                self._summed_attachment_log_like = logsumexp(self._attachment_log_like, axis=0)
            else:
                summation_weights = np.array([1, 1, -1])[:, np.newaxis]
                for node in self._changed_nodes:
                    for attach_idx, log_like in self._update_attachment_log_like_from(node):
                        summands = np.vstack(
                            [self._summed_attachment_log_like,
                             log_like,
                             self._attachment_log_like[attach_idx]]
                        )
